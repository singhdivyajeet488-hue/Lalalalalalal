...
try {
  ...
  try {
    ...
    console.log('✅ Using libsodium-wrappers for encryption');
  } catch {
    require('tweetnacl');
  ...
  try {
    switch (commandName) {
      ...
      case 'play': {
        ...
        // Join voice first, wait until Ready
        ...
        if (!connection || connection.state.status === VoiceConnectionStatus.Destroyed) {
          ...
          try {
            ...
            connection.destroy();
            return interaction.editReply('❌ Failed to connect to voice channel. Please try again.').catch(() => {});
          }
        }
      ...
      case 'nowplaying': {
        ...
        const embed = new EmbedBuilder().setColor('#5865F2').setTitle('🎵 Now Playing')
          ...
          .addFields(
            ...
            { name: '🔁 Loop', value: ['Off','Song','Queue'][q.repeatMode], inline: true }
      ...
      case 'loop': {
        ...
        interaction.reply(`🔁 Loop set to **${['Off','Song','Queue'][mode]}**`);
        break;
      }
